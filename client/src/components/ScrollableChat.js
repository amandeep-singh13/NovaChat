                    ...
                    >
                        ...
                        <div style={{ display: "flex", marginTop: "5px" }}>
                            <IconButton
                                ...
                                variant={m.reactions && m.reactions.some(r => r.reactionType === 'like') ? 'solid' : 'outline'}
                            />
                            <IconButton
                                icon={<RiThumbDownLine style={{ color: 'red'}} />}
                                aria-label="Dislike"
                                onClick={() => handleReact(m._id, 'dislike')} // handleReact function to be implemented
                                variant={m.reactions && m.reactions.some(r => r.reactionType === 'dislike') ? 'solid' : 'outline'}
                            />
                            <IconButton
                                icon={<RiHeartLine style={{color: '#ff1493'}}/>}
                                aria-label="Heart"
                                onClick={() => handleReact(m._id, 'heart')} // handleReact function to be implemented
                                variant={m.reactions && m.reactions.some(r => r.reactionType === 'heart') ? 'solid' : 'outline'}
                            />
                        </div>
                    </span>
                </div>
